...
def run_single_simulation(run_id, temp_dir="temp_runs"):
    """
    Runs a single simulation of the DEJMPS protocol.
...
def main():
    parser = argparse.ArgumentParser(description='Run multiple simulations of the DEJMPS protocol')
    parser.add_argument('--runs', type=int, default=100, help='Number of protocol runs')
    parser.add_argument('--output', type=str, default='dejmps_results.json', help='Output file for results')
    ...
    print(f"Running DEJMPS protocol simulation for {num_runs} iterations...")
    ...
    if results["successful_runs"] > 0:
        try:
            ...
            plt.title('Distribution of Fidelities in successful DEJMPS protocol runs')
            plt.xlabel('Fidelity')
