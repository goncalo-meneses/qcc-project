...
def run_single_simulation(run_id, temp_dir="temp_runs"):
    """
    Runs a single simulation of the EPL protocol.
...
def main():
    parser = argparse.ArgumentParser(description='Run multiple simulations of the EPL protocol')
    parser.add_argument('--runs', type=int, default=100, help='Number of protocol runs')
    parser.add_argument('--output', type=str, default='epl_results.json', help='Output file for results')
    ...
    print(f"Running EPL protocol simulation for {num_runs} iterations...")
    ...
    if results["successful_runs"] > 0:
        try:
            ...
            plt.title('Distribution of Fidelities in successful EPL protocol runs')
            plt.xlabel('Fidelity')
